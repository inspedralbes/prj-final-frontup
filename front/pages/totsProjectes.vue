  ...
  <div class="container">
    ...
    <div v-else>
      ...
      <div class="pagination">
        ...
        <span>Pàgina {{ currentPage }} de {{ lastPage }}</span>
        <button @click="nextPage" :disabled="currentPage === lastPage" class="page-btn">Següent</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchProjects() {
      this.loading = true;
      try {
        const url = new URL("http://localhost:8000/api/projects/all");
